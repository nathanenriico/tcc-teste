document.addEventListener("DOMContentLoaded", function () {
    ...
    // Filtra dados e atualiza gráficos
     filterBtn.addEventListener("click", function () {
        ...
        // Cria objetos Date (assumindo que os inputs são do tipo date, no formato ISO: YYYY-MM-DD)
        const startDate = new Date(startDateStr);
        const endDate = new Date(endDateStr);
        
        // Opcional: Ajusta as datas para ignorar o horário (define tempo início e fim do dia)
        startDate.setHours(0, 0, 0, 0);
        endDate.setHours(23, 59, 59, 999);

        // Se as datas não forem válidas, alerta o usuário
        if (isNaN(startDate) || isNaN(endDate)) {
            alert("Por favor, selecione datas válidas.");
            return;
        }

        // Filtra os dados de vendas
